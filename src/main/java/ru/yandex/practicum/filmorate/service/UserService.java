...
public class UserService {
    ...
    public Collection<User> addFriend(Integer firsFriend, Integer secondFriend) { // Дружба односторонняя. Для хранений состояния дружбы я использовал мапу
        ...
        if (checkSecondInFirst.isEmpty() && checkFirstInSecond.isEmpty()) { // У удаления из друзей похожая логика, Если пользователи в друзьях, то у первого пользователя второй просто удаляется, а у второго в мапе меняется статус на отправил запрос
